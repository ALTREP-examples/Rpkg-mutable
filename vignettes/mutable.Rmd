...
- mutables cause problems with arithmetic optimizations and maybe others
- duplicate not necessarily producing a standard vector may also
  - a test that rules out ALTREPs would do
  - but we might want some ALTREPs to be re-usable
  - maybe a Can_reuse method is needed?
- duplicate-and-modify code won't work right -- how common is that?
- wrapped mutables is also a problem
- distinguish internally, externally mutable?
  - explicitly mutable at the R level
  - standard R, but mutating may mutate external store
  - standard R, no mutation fro R, but external mutation can happen

provides an implementation of mutable vectors in R as a simple
illustration of the `ALTREP` framework.
